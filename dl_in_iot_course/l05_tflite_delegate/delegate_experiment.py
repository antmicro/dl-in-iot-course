...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--delegate-path',
        help='Path to the model delegate',
        type=Path,
        required=True
    )
    parser.add_argument(
        '--num-tests',
        help='Number of tests to conduct',
        type=int,
        default=1000
    )
    ...
    convert_onnx_to_tensorflow(
        args.input_onnx_model_path,
        args.tensorflow_model_path
    )
    ...
    nodelegate = tf.lite.Interpreter(
    ...
    for _ in tqdm(range(args.num_tests)):
        ...
        delegate.invoke()
        delegateres = delegate.get_tensor(
            delegate.get_output_details()[0]['index']
        )
